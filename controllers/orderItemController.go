...
func GetOrderItems() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while fetching ordered item"})
		}
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	lookupStage := bson.D{{"$lookup", bson.D{{"from", "food"}, {"localField", "food_id"}, {"foreinField", "food_id"}, {"as", "food"}}}}
	...
	projectStage := bson.D{
		{"$project", bson.D{
			...
			{"table_number", "$tabel.table_number"},
...
func CreateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		var orderItemPark OrderItemPark
		var order models.Order
		if err := c.BindJSON(&orderItemPark); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
		order.Order_Date, _ = time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
		orderItemsToBeInserted := []interface{}{}
		order.Table_id = orderItemPark.Table_id
		order_id := OrderItemOrderCreator(order)
		for _, orderItem := range orderItemPark.Order_items {
